/**
 * @summary Given wishlist items array, a variant ID, and the prices for that variant,
 *   returns an updated wishlist items array. Updates `price`, `compareAtPrice`, and `subtotal`
 *   as necessary. You need not know for sure whether prices have changed since
 *   the wishlist items were last updated. The return object will have a `didUpdate`
 *   boolean property that you can check to see whether any changes were made.
 * @param {Object[]} items Wishlist items
 ...
 */
export default function updateWishlistItemsForVariantChanges(items, catalogProductVariant, prices) {
  let didUpdate = false;
