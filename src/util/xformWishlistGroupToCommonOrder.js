...
/**
 * @param {Object} wishlist A wishlist
 * @param {Object} group The wishlist fulfillment group
 * @param {Object} context App context
 * @returns {Object} Valid CommonOrder from a wishlist group
 */
export default async function xformWishlistGroupToCommonOrder(wishlist, group, context) {
  const { collections } = context;
  const { accountId, currencyCode } = wishlist;

  let items = group.itemIds.map((itemId) => wishlist.items.find((item) => item._id === itemId));
  ...
  // Discounts are stored as the sum of all discounts, per wishlist. This will need to be updated when we refactor discounts to go by group.
  const discountTotal = wishlist.discount || 0;
  ...
  const totals = {
    groupDiscountTotal: {
      ...
      currencyCode: wishlist.currencyCode
    },
    groupItemTotal: {
